...
def traitementJointureAttributListe(source, jointure, sortie, ma_liste=[MonParcellaireCavePourApport]):
	"""
	processing.run("native:joinattributestable", {'INPUT':'xx','FIELD':'nom','INPUT_2':'ccc','FIELD_2':'Code Vigne',
    'FIELDS_TO_COPY':['cave pour apport'],'METHOD':1,'DISCARD_NONMATCHING':False,'PREFIX':'','OUTPUT':'TEMPORARY_OUTPUT'})
	"""
	algo_name,  algo_simplifie ="native:joinattributestable",  "Jointure des attributs cave ..."
	result = processing.run(algo_name, 
		{'INPUT': source, 'FIELD':'nom', 'INPUT_2':jointure, 'FIELD_2':'Code Vigne', 'FIELDS_TO_COPY':ma_liste, \
		'METHOD':1,'DISCARD_NONMATCHING':False,'PREFIX':'', \
		'OUTPUT': sortie})
	if result == None:
		monPrint( "Erreur bloquante durant processing {0}".format( algo_simplifie), T_ERR)
...
class MonParcellaireDockWidget(QtWidgets.QDockWidget, FORM_CLASS):
	...
	def fusionnerJointure(self, cheminCompletParcelle, jointureChoisie):
		...
		return jointureChoisie, attributsAJoindreOrdonne, affectation_sans_suite
	...
	def extraireVignesMesParcelles(self):
		...
		try:
			...
			available_cave=dfVignes['cave_transfert'].sort_values().unique()
			monPrint("= Info = Les caves {}.".format( available_cave))
	...
	def slotTraiterRepertoireGPKGJointure( self):
		...
		if CHOIX_JOINTURE == "YES":
			...
			if jointureChoisie != None:
				...
				CHEMIN_JOINTURE,  attributsAJoindreOrdonne, affectation_sans_suite = \
				...
				if CHOIX_SUITE == "YES":
					# Récupérer la cave pour apport par un jointure supplémentaire
					nom_cave_pour_apport=os.path.join( REPERTOIRE_GPKG, MonParcellaire_JOI_CAVE+EXT_csv)
					if os.path.isfile( nom_cave_pour_apport):
						monPrint("= Info = Jointure pour cave d'apport: {}".format( nom_cave_pour_apport))
						monProjet, nouveauGroupeCaveApport = self.ouvrirProjetETGroupe( MonParcellaire_AFF_SANS_SUITE_CAVE)
						_, csv, nomCsv = self.rechercherDelimiteurJointure( nom_cave_pour_apport, "No Pandas")
						monProjet.addMapLayer(csv, False)
						nouveauGroupeCaveApport.addLayer( csv)
						CHEMIN_SYNCHRONISATION = quel_chemin_synchro(REPERTOIRE_GPKG, REP_SYN)
						affectation_sans_suite_mais_cave = os.path.join( CHEMIN_SYNCHRONISATION, MonParcellaire_AFF_SANS_SUITE_CAVE+EXT_geojson)
						traitementJointureAttributListe(affectation_sans_suite, csv, affectation_sans_suite_mais_cave)
						# Ouvrir le resultat
						sans_suite_et_cave = QgsVectorLayer(affectation_sans_suite_mais_cave, MonParcellaire_AFF_SANS_SUITE_CAVE, "ogr")
						monProjet.addMapLayer(sans_suite_et_cave, False)
						nouveauGroupeCaveApport.addLayer( sans_suite_et_cave)
						self.affecterVignesSuites( MonParcellaire_AFF_SANS_SUITE_CAVE+EXT_geojson, 'OUI')
					else:
	...
	def affecterVignesSuites( self, source=MonParcellaire_AFF_SANS_SUITE+EXT_geojson, \
			choix_cave_pour_apport='NON', \
			cible = MonParcellaire_SUI+ SEP_U +'SANS_superficie'+EXT_geojson):
		""" Deviner les affectations pour les vignes suites (mais aussi cepage, orientation et Cave pour apport 
		"""
		REPERTOIRE_GPKG = self.Repertoire_lineEdit.text()
		CHEMIN_SYNCHRONISATION = quel_chemin_synchro(REPERTOIRE_GPKG, REP_SYN)
		if choix_cave_pour_apport == 'OUI':
			monPrint("{} = Traiter la cave d'apport.".format( E_CLAP))
		else:
			monPrint("{} = Ne pas traiter la cave d'apport.".format( E_CLAP))
		...
		for pos, une_parcelle in enumerate( available_parcelles):
			...
			try:
				...
				orientation=9999
			#monPrint( "Pour la parcelle {} a pour affectation {} et orientation {}".format(une_parcelle, affectation_coopviti, orientation))
			# Rapprochement des vignes suites (A B C D) avec sa vigne principale
			suite="Non"
			cave="Inconnue"
			if choix_cave_pour_apport == 'OUI':
				cave= df_une_parcelle['Cave pour apport'].values[0]
				#monPrint( "Pour la parcelle {} la cave est {}".format(une_parcelle, cave))
			...
			if affectation_coopviti is None or affectation_coopviti == "nan" or len(affectation_coopviti) <= 0:
				...
				if caractere_suite < 0:    # parcelle suite possible
					...
					if une_parcelle[0:caractere_suite] == derniere_liste_a_ecrire[0]:
						#monPrint("== Parcelle suite {} est rapprochée de {} pour les valeurs {}".\
						#	format( une_parcelle, derniere_liste_a_ecrire[0], derniere_liste_a_ecrire ))
						affectation_coopviti= derniere_liste_a_ecrire[1]
						cepage=derniere_liste_a_ecrire[2]
						if orientation==9999:
							orientation=int( derniere_liste_a_ecrire[3])
						if choix_cave_pour_apport == 'OUI':
							cave=derniere_liste_a_ecrire[4]
						suite="Oui"
					else:
			...
			# Memoriser si non suite
			if une_parcelle[-1] not in ["A", "B", "C", "D", "E", "F", "a", "b", "c", "d", "e", "f"]:
				if choix_cave_pour_apport == 'OUI':
					derniere_liste_a_ecrire = [une_parcelle, affectation_coopviti, cepage, str(orientation), cave]
				else:
					derniere_liste_a_ecrire = [une_parcelle, affectation_coopviti, cepage, str(orientation)]
				#monPrint("= Info = {} Derniere memorisée {}".format( une_parcelle, derniere_liste_a_ecrire))
